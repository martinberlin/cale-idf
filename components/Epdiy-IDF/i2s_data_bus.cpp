...
/// static : Initializes a DMA descriptor.
void I2SDataBus::fill_dma_desc(volatile lldesc_t *dmadesc, uint8_t *buf,
                          i2s_bus_config *cfg) {
...
uint32_t I2SDataBus::dma_desc_addr() {
  return (uint32_t)(current_buffer ? i2s_state.dma_desc_a
                                   : i2s_state.dma_desc_b) &
         0x000FFFFF;
}

// static_ Set up a GPIO as output and route it to a signal.
 void I2SDataBus::gpio_setup_out(int gpio, int sig, bool invert) {
  if (gpio == -1)
    return;
...
// volatile
volatile uint8_t* IRAM_ATTR I2SDataBus::i2s_get_current_buffer() {
  return current_buffer ? i2s_state.dma_desc_a->buf : i2s_state.dma_desc_b->buf;
}

bool IRAM_ATTR I2SDataBus::i2s_is_busy() {
  // DMA and FIFO must be done
  return !output_done || !I2S1.state.tx_idle;
}

void IRAM_ATTR I2SDataBus::i2s_switch_buffer() {
...
void IRAM_ATTR I2SDataBus::i2s_start_line_output() {
  output_done = false;
  ...
  gpio_set_level((gpio_num_t)start_pulse_pin, 0);
  dev->conf.tx_start = 1;
}

void I2SDataBus::i2s_bus_init(i2s_bus_config *cfg) {
