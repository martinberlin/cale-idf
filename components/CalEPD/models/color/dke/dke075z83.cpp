...
DRAM_ATTR const epd_init_4 Dke075Z83::epd_wakeup_power={
0x01,{
  0x07,
...
DRAM_ATTR const epd_init_1 Dke075Z83::epd_panel_setting={
0x00,{
  0x0F},1
};

DRAM_ATTR const epd_init_4 Dke075Z83::epd_resolution={
0x61,{
  DKE075Z83_WIDTH / 256, //source 800
  DKE075Z83_WIDTH % 256,
  0x01, //gate 448
  0xE0
},4};
...
// Constructor
Dke075Z83::Dke075Z83(EpdSpi& dio): 
  Adafruit_GFX(DKE075Z83_WIDTH, DKE075Z83_HEIGHT),
  Epd(DKE075Z83_WIDTH, DKE075Z83_HEIGHT), IO(dio)
{
  printf("Dke075Z83() constructor injects IO and extends Adafruit_GFX(%d,%d)\n",
  DKE075Z83_WIDTH, DKE075Z83_HEIGHT);  
}

//Initialize the display
void Dke075Z83::init(bool debug)
{
    debug_enabled = debug;
    if (debug_enabled) printf("Dke075Z83::init(debug:%d)\n", debug);
    //Initialize SPI at 4MHz frequency. true for debug
    IO.init(4, debug);
    fillScreen(EPD_WHITE);
}

void Dke075Z83::fillScreen(uint16_t color)
{
  uint8_t black = DKE075Z83_8PIX_WHITE;
  uint8_t red = DKE075Z83_8PIX_RED_WHITE;
  if (color == EPD_RED) {

  } else if (color == EPD_BLACK) {
    black = DKE075Z83_8PIX_BLACK;
    printf("fillScreen BLACK SELECTED\n");
  } else if (color == EPD_WHITE) {
    red = DKE075Z83_8PIX_RED;
...
void Dke075Z83::update()
{
  ...
  uint16_t i = 0;
  uint8_t xLineBytes = DKE075Z83_WIDTH/8;
  uint8_t x1buf[xLineBytes];

  IO.cmd(0x10);
  printf("Sending a %d bytes buffer via SPI\n", (int)DKE075Z83_BUFFER_SIZE);  
  
  for(uint16_t y =  1; y <= DKE075Z83_HEIGHT; y++) {
...
void Dke075Z83::updateWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h, bool using_rotation)
{
  printf("Color epapers from Goodisplay do not support partial update. Full update triggered\n");
  update();
}

void Dke075Z83::_waitBusy(const char* message){
  if (debug_enabled) {
...
void Dke075Z83::_rotate(uint16_t& x, uint16_t& y, uint16_t& w, uint16_t& h)
{
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      swap(w, h);
      x = DKE075Z83_WIDTH - x - w - 1;
      break;
    case 2:
      x = DKE075Z83_WIDTH - x - w - 1;
      y = DKE075Z83_HEIGHT - y - h - 1;
      break;
    case 3:
      swap(x, y);
      swap(w, h);
...
void Dke075Z83::drawPixel(int16_t x, int16_t y, uint16_t color) {
  if ((x < 0) || (x >= width()) || (y < 0) || (y >= height())) return;
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      x = DKE075Z83_WIDTH - x - 1;
      break;
    case 2:
      x = DKE075Z83_WIDTH - x - 1;
      y = DKE075Z83_HEIGHT - y - 1;
      break;
    case 3:
      swap(x, y);
      y = DKE075Z83_HEIGHT - y - 1;
  ...
  uint16_t i = x / 8 + y * DKE075Z83_WIDTH / 8;
