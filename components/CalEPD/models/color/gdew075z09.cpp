...
DRAM_ATTR const epd_init_2 Gdew075z09::epd_wakeup_power={
0x01,{
  0x37,
  0x00},2
};

DRAM_ATTR const epd_init_2 Gdew075z09::epd_panel_setting={
0x00,{
  0xCF,
  0x08},1
};

DRAM_ATTR const epd_init_3 Gdew075z09::epd_boost={
0x06,{0xC7,0xCC,0x28},3
};

DRAM_ATTR const epd_init_4 Gdew075z09::epd_resolution={
0x61,{
  0x02, //source 600=0x258
  0x80,
  0x01, //gate 448
  0x80
},4};
...
// Constructor
Gdew075z09::Gdew075z09(EpdSpi& dio): 
  Adafruit_GFX(GDEW075Z09_WIDTH, GDEW075Z09_HEIGHT),
  Epd(GDEW075Z09_WIDTH, GDEW075Z09_HEIGHT), IO(dio)
{
  printf("Gdew075z09() constructor injects IO and extends Adafruit_GFX(%d,%d)\n",
  GDEW075Z09_WIDTH, GDEW075Z09_HEIGHT);  
}

//Initialize the display
void Gdew075z09::init(bool debug)
{
    debug_enabled = debug;
    if (debug_enabled) printf("Gdew075z09::init(debug:%d)\n", debug);
    //Initialize SPI at 4MHz frequency. true for debug
    IO.init(4, debug);
    fillScreen(EPD_WHITE);
}

void Gdew075z09::fillScreen(uint16_t color)
{
  uint8_t black = 0x00;
  uint8_t red = 0x00;
  if (color == EPD_WHITE);
...
void Gdew075z09::updateWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h, bool using_rotation)
{
  printf("Color epapers from Goodisplay do not support partial update. Full update triggered\n");
  update();
}

void Gdew075z09::_waitBusy(const char* message){
  if (debug_enabled) {
...
void Gdew075z09::_rotate(uint16_t& x, uint16_t& y, uint16_t& w, uint16_t& h)
{
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      swap(w, h);
      x = GDEW075Z09_WIDTH - x - w - 1;
      break;
    case 2:
      x = GDEW075Z09_WIDTH - x - w - 1;
      y = GDEW075Z09_HEIGHT - y - h - 1;
      break;
    case 3:
      swap(x, y);
      swap(w, h);
...
void Gdew075z09::drawPixel(int16_t x, int16_t y, uint16_t color) {
  if ((x < 0) || (x >= width()) || (y < 0) || (y >= height())) return;
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      x = GDEW075Z09_WIDTH - x - 1;
      break;
    case 2:
      x = GDEW075Z09_WIDTH - x - 1;
      y = GDEW075Z09_HEIGHT - y - 1;
      break;
    case 3:
      swap(x, y);
      y = GDEW075Z09_HEIGHT - y - 1;
  ...
  uint16_t i = x / 8 + y * GDEW075Z09_WIDTH / 8;
 ...
 * 0x00 BLACK
 */
void Gdew075z09::_send8pixel(uint8_t black, uint8_t red)
{ 
